...
class MtsHomeOnlineSecondPage(BasePage):
    ...
    def send_popup_application_connection_your_address(self):
        with allure.step("Заполнить попап и отправить заявку"):
            self.page.locator(ApplicationPopupCheckConnectionSecond.ADDRESS_INPUT_SECOND).fill("Тестимя")
            time.sleep(2)
            ...
            while retry_count < max_retries:
                try:
                    ...
                    try:
                        ...
                        allure.attach(f"Обычный клик не удался, пробуем через JavaScript: {str(click_error)}",
                                      "warning")
                    ...
                    if retry_count == max_retries:
                        allure.attach(
                            f"Не удалось проверить ссылку {link_name} после {max_retries} попыток. Ошибка: {str(e)}",
                            "error")
                        raise
