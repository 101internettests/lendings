...
class TestMoskvaRTKOnlineHomeInternet:
    @allure.title("15. Отправка заявки из попапа Выгодное спецпредложение! по нажатию фиксированной красной кнопки "
                  "звонка в правом нижнем углу")
    def test_application_popup_super_offer_red_button(self, page_fixture, msk_rtk_online_home_inter):
        page = page_fixture
    ...
    # def test_application_popup_button_connect(self, page_fixture, msk_rtk_online_home_inter):
    #     page = page_fixture
    #     page.goto(msk_rtk_online_home_inter)
    #     mts_page = MtsHomeOnlinePage(page=page)
    #     region_page = ChoiceRegionPage(page=page)
    ...
    #     mts_page.close_thankyou_page()
    #
    # @allure.title("18. Отправка заявки со ВСЕХ  форм на странице с названием Проверить возможность подключения "
    #               "Ростелеком по вашему адресу")
